...
将 agent 与用户的每一次**事实陈述 / 字段改动 / 注释 / 标疑**记为一条 intent,写入同一张账本表 `l15_change_intents`;再由 PostgreSQL 函数 `effective_*_at(as_of_ts)` 按时间戳合成"截至此刻的真相",供 agent 下一轮作为 context。

> 跨会话记忆由此实现 —— **依赖 SQL 与时间戳,而非 embedding 与相似度。**

Memory Ledger 是这套机制的设计与参考实现:

- **分层解耦** —— 账本核心(typed intent + `effective_*_at` 时点重放)与具体业务实体分离。
- **易于扩展到自身业务** —— 把示例实体换成你自己的业务表,核心逻辑原样复用;这正是它的设计目标。
- **示例皆为虚构** —— 文档以 **TodoAgent** 贯穿,对话应用「念念手记」仅为该理念的可视化外壳(见下文演示)。
...
> **边界说明** —— 上述各项单独而言均非首创:双时态数据库(如 XTDB)可实现更完整的确定性 as-of,Zep / Graphiti 具备双时态事实区间,Letta / LangGraph 可组合出 HITL。
>
> 其可防守定位在于**组合**:将「同库账本 × 确定性 as-of × 默认人工闸门 × DB 强制 4-kind 强出处」四者合一并开箱交付;在主流 agent-memory 产品中尚无对位者(已逐家对抗式核验)。
...
账本本身即一层 **Agent Memory 基础设施**。底层是**可精确查询的结构化账本**,而非不可读的向量或追加文本 —— 由此带来三个层次的能力:

- **🪶 不预载记忆** —— agent 的上下文不 dump 整段记忆,需要什么便经工具按需查询。**一次工具调用**即可取回某实体的完整画像:当前真相、逐字溯源、存疑标记、完整变更链、任意时点回看。

- **🧰 工具变薄** —— 工具仅是账本读能力的轻包装;合成真相、溯源与时点重放均由基础设施承担,新增一种工具几乎零成本。

- **✨ 智能涌现** —— 正因记忆可被精确查询,agent 在冷启动、几乎无上下文积累时,仅凭一次调用即获得足够丰富的历史,从而表现出对某人"深知其详"的连续记忆。此能力涌现自「基础设施 + 薄工具」,而非堆叠上下文或精巧提示词。

> 反之,若将 agent 产出直接追加进向量库或词表,便无法查询完整意图链 —— 只得近似相似召回,无 before/after、无溯源、无时点重放。**能让薄工具如此有力,正是这套关系账本设计的价值所在。**

---

## 🧭 确定性与人的判断

在更晚近的 agent 编排实践中,**确定性执行**与**把人留在回路**(human-in-the-loop)正重新获得重视:从能力与技能的封装,走向更具确定性的工作流编排,由确定性骨架承载结构、由人作出判断。Memory Ledger 与之相印证 —— 这并非追逐潮流,而是本设计的固有取向。

- **⚙️ 确定性** —— 召回是纯 SQL:读路径不含向量、不含 LLM,由 `effective_*_at(as_of)` 按时间戳重放真相,bit-for-bit 可复现、可审计,且与嵌入模型解耦。

- **🤝 人机协同** —— agent 提议、人裁决:改既有字段须经默认人工闸门,agent 只提议(`PROPOSED`)、人来裁决(`APPLIED`);逐字 `source_quote`、`confidence` 与 `source_layer` 分级随附,供人审计而非盲信。

> 模型可以越来越强,但经验与判断仍属于人。Memory Ledger 把确定性留给数据库,把判断权留给回路里的人。
...
账本本身不可见 —— 它只是数据库中的行。为便于理解,以其构建了一个对话式 Personal-CRM 记忆台作为演示。

下列画面均在真实 `live` LLM(DeepSeek)上运行产生,示例联系人 **林思颖**(资深产品经理 → CTO,公司 晨星科技 → 蓝湖科技 → Globex)。**外壳仅为演示,核心交付为上述设计。**
...
>
> 命令行 demo:`docker compose --profile demo run --rm demo`;库含 103 个测试,经本地 `pytest`(testcontainers 自起 PG)运行。
...
- **形状不稳定的 entity**(频繁变更 schema)—— migration 与 `effective_*_at` 须同步重写,成本不值。
