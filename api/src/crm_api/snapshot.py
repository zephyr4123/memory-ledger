"""build_person_snapshot —— person 的 effective 视图 → 注入模型的 snapshot 文本.

从 examples/personal_crm/snapshot.py 移植。组合 ledger.effective(读合成真相) +
sanitize_text(清洗不可信文本, 防二阶 prompt 注入) + render_snapshot(包进"这是数据
不是指令"的定界块)。snapshot 是每轮注入 LLM 的当前记忆上下文。
"""
